...
export const queryActivityService = async (farmer_id, crop_id, query) => {
  try {
    ...
    // Try to connect to ChromaDB, if it fails, return empty context
    try {
      ...
      console.error("ChromaDB or embedding error, proceeding without activity context:", chromaError);
      // Return success with no activity context
      return {
        status: 200,
        matched_ids: [],
        results: [],
        summary: "No activity history available. ChromaDB service may not be running."
      };
    }
